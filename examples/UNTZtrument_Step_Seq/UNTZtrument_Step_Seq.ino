...
void save(void)
{
  ...
  EEPROM.write(address + 3, 0x03); // version 2
  address += 4;
...
void load(void)
{
  ...
  (EEPROM.read(address + 1) == 0xaa) &&
    ...
    (EEPROM.read(address + 3) == 0x03)) // version 2
  {
    address += 4;
...
void loop() 
{
  ...
  if((t - prevBeatTime) >= beatInterval[playing_grid_set]) // Next beat?
  {
    ...
    tempo[visible_grid_set] = eTempo.getValue() / 16 ; // Div4 for encoder detents and Div4 byte storage
    beatInterval[visible_grid_set] = 15000L / tempo[visible_grid_set];
  }
