...
Improvements include:
	implements up to 16 beats on the grid with only an 8x8 untz using one of the encoders for scrolling left and right
	the number of beats on the grid is adjustable up to 16
	implements up to 8 layers outputing notes on different channels at the same time
	implements up to 8 sets of sequence grid sets with 8 layers each that can be swiched
	can change to and instruments (press button for first set 1-64, press again for second set 65-127)
	has embedded notes scales (major, minor, chromatic, etc)
	can adjust tempo and volume using 2 of the encoders
	can adjust position on the scale with the last encoder
	saves current state of the sequencer in EEPROM when button is held down for 10 seconds
